...
export default function TeamPage() {
  ...
  const TeamSection = ({ title, members, bgColor = "bg-white dark:bg-dark-surface", id }: { title: string, members: any[], bgColor?: string, id?: string }) => {
    // MH brand border colors alternating between Hunter Green and Leather Tan
    ...
    <section className={`py-16 ${bgColor} dark:bg-dark-surface transition-colors duration-300`} id={id}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl md:text-4xl font-bold text-mh-hunter-green dark:text-dark-text text-center mb-12">
          {title}
        </h2>
        <div className={`grid gap-8 ${
          ...
          {members.map((member, index) => (
            <div key={index} className={`group relative rounded-xl hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 flex flex-col h-full border-2 ${getBorderColor(index)} dark:border-opacity-80 hover:border-opacity-100`}>
              {/* Professional card with clean design */}
              <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl h-full relative overflow-hidden border border-gray-200 dark:border-dark-border transition-all duration-300 group-hover:from-gray-50 group-hover:to-gray-100 dark:group-hover:from-gray-700 dark:group-hover:to-gray-800 flex flex-col shadow-lg group-hover:shadow-xl">
                {/* Image with professional hover effects */}
                <div className="aspect-square relative overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-mh-hunter-green/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300"></div>
                </div>
                
                {/* Card content */}
                <div className="p-6 pb-0 flex flex-col flex-grow">
                  <h3 className="text-xl font-bold text-mh-hunter-green dark:text-dark-text mb-1 group-hover:text-mh-leather-tan dark:group-hover:text-mh-leather-tan transition-colors duration-300">{member.name}</h3>
                    ...
                    {getSkillTags(member).map((skill, skillIndex) => (
                      <span 
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-300 transform hover:scale-105 shadow-sm hover:shadow-md ${
                          skillIndex % 2 === 0 
                            ? 'bg-mh-hunter-green/10 text-mh-hunter-green hover:bg-mh-hunter-green/20 border border-mh-hunter-green/20' 
                            : 'bg-mh-leather-tan/10 text-mh-leather-tan hover:bg-mh-leather-tan/20 border border-mh-leather-tan/20'
                        }`}
                      >
                        {skill}
                      </span>
                ...
                {/* MH brand contact buttons */}
                <div className="flex justify-center space-x-4 mt-auto pb-6 pt-4">
                  <a
                    href={`mailto:${member.email}`}
                    className="flex items-center justify-center w-12 h-12 bg-mh-hunter-green text-white rounded-full transition-all duration-300 hover:bg-mh-leather-tan hover:scale-110 hover:-translate-y-1 shadow-lg hover:shadow-xl transform group/btn"
                  ...
                  <a
                    ...
                    className="flex items-center justify-center w-12 h-12 bg-mh-leather-tan text-white rounded-full transition-all duration-300 hover:bg-mh-hunter-green hover:scale-110 hover:-translate-y-1 shadow-lg hover:shadow-xl transform group/btn"
  ...
  <TeamSection title="Administration & Support" members={administration} bgColor="bg-pale-gray dark:bg-dark-surface-2" id="administration" />
      ...
      {/* MH brand Join Our Team CTA */}
      <section className="py-20 bg-gradient-to-br from-mh-hunter-green via-mh-hunter-green-dark to-mh-leather-tan relative overflow-hidden">
